...
from .TimeMix import RWKV_Tmix_x070
# import cv2
...
class IG_MSA_RWKV7(nn.Module):
    def __init__(
        ...
        args.head_size = dim_head
        args.dim_att = dim_head * heads
        args.n_embd = dim_head * heads
        # print(args)
        assert args.dim_att % args.head_size == 0
        
        self.rwkv_attention = RWKV_Tmix_x070(args).to(torch.bfloat16)
    ...
    def forward(self, x_in, illu_fea_trans):
        ...
        b, h, w, c = x_in.shape
        x_original = x_in
        # print(x_in.shape, illu_fea_trans.shape)
        
        x = x_in.reshape(b, h * w, c) #  [b, h*w, c]
        x_illu = illu_fea_trans.reshape(b, h * w, c)
        
        if x.dtype != torch.bfloat16:
            x = x.to(torch.bfloat16).contiguous()
        if x_illu.dtype != torch.bfloat16:
            x_illu = x_illu.to(torch.bfloat16).contiguous()

        attn_out = self.rwkv_attention(x, x_illu)
        
        out_c = attn_out.view(b, h, w, c)

        out = out_c + x_original # residual
...
class IGAB(nn.Module):
    def __init__(
        ...
        for _ in range(num_blocks):
            # print(f"IGAB: {dim}, {dim_head}, {heads}")
...
if __name__ == '__main__':
    ...
    stats = summary(model, input_data=inputs, verbose=0)
    print(stats)
    print(f"Total Parameters: {stats.total_params / 1e6:.2f} M")
    print(f"Total GFLOPs: {stats.total_mult_adds / 1e9:.2f} GFLOPs")
    output = model(inputs)
