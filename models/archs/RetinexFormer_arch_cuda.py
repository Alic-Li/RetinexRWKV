...
from rwkvfla.layers import RWKV7Attention
from rwkv7_cuda.rwkv_v7_demo import RWKV7_OP
...
class IG_MSA_RWKV7(torch.jit.ScriptModule):
...
#         args.head_size_a = dim_head
#         args.n_embd = dim_head * heads
#         args.dim_att = dim_head * heads
#         args.dim_ffn = dim_head * heads * 4
#         self.rwkv_attention = RWKV7Block(args,layer_id=0)
...
#         """
#         b, h, w, c = x_in.shape
#         x = x_in.permute(0, 3, 1, 2).contiguous().view(b, c, -1).permute(0, 2, 1)  # [b, h*w, c]
#         illu_attn = illu_fea_trans.permute(0, 3, 1, 2).contiguous().view(b, c, -1).permute(0, 2, 1)  # [b, h*w, c]

#         # 将光照特征与输入特征结合
#         x = x + illu_attn
#         print(x.shape, illu_attn.shape)

#         x, _= self.rwkv_attention(x = x, v_first = x)
#         x = x.permute(0, 2, 1).contiguous().view(b, c, h, w).permute(0, 2, 3, 1)  # [b, h, w, c]
#         return x
    ...
    def __init__(
        ...
        for _ in range(num_blocks):
            self.blocks.append(nn.ModuleList([
                IG_MSA_RWKV7(dim=dim, dim_head=dim_head, heads=heads),
...
if __name__ == '__main__':
    model = RetinexFormer(stage=1,n_feat=12,num_blocks=[1,2,2]).cuda()
    # print(model)
